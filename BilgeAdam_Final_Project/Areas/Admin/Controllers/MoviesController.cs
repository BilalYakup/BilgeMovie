...
namespace BilgeAdam_Final_Project.Areas.Admin.Controllers
{
    ...
    public class MoviesController : Controller
    {
        ...
        {
            //Kategori listesi ve film listesi ModelState.IsValid olmadığı zaman Boş dönmesin diye ViewBag'leri buraya da yazıyoruz.
            ViewBag.Categories = new SelectList
               (
                   await _categoriesRepo.GetAllAsync(), "Id", "Name"
               );
        ...
        public async Task<IActionResult> UpdateMovie(int id)
        {
            ...
            if (movie != null)
            {
                ...
                foreach (var category in await _categoriesRepo.GetAllAsync())
                {
                    if (_movieCategoriesRepo.GetAllByMovieId(movie.Id).Result.Any(x => x.CategoryId == category.Id))
                    {
